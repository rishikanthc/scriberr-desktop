...
export function AudioVisualizer({
	...
	useEffect(() => {
		...
		// Top: Amber (accent-text)
		gradient.addColorStop(0, "#FFAB40");
		// Mid: Primary Orange (accent-primary)
		gradient.addColorStop(0.5, "#FF6D1F");
		// Bottom: Deep Red (accent-hover)
			...
			for (let i = 0; i < barCount; i++) {
				...
				if (activeTiles > peakPositionsRef.current[i]) {
					...
					if (peakDropsRef.current[i] > 5) {
						peakPositionsRef.current[i] = Math.max(
							0,
							peakPositionsRef.current[i] - 1,
						);
						peakDropsRef.current[i] = 0;
					}
				}
				...
				for (let j = 0; j < maxTilesColumn; j++) {
					...
					if (j < activeTiles) {
						...
						ctx.globalAlpha = 0.8; // Slight transparency for layering feel
						ctx.beginPath();
						ctx.roundRect(x, y, TILE_SIZE, TILE_SIZE, 1 * SCALE);
						ctx.fill();
						ctx.globalAlpha = 1.0;
					} else if (j === peakTile && peakTile > 0 && isPlaying) {
						ctx.fillStyle = "#FFAB40"; // Top of gradient (Amber)
						ctx.globalAlpha = 0.5;
						ctx.beginPath();
						ctx.roundRect(x, y, TILE_SIZE, TILE_SIZE, 1 * SCALE);
						ctx.fill();
